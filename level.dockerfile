...
RUN git clone --recursive https://github.com/google/leveldb.git /opt/leveldb

RUN mkdir -p /opt/leveldb/build
WORKDIR /opt/leveldb/build
...
RUN make VERBOSE=1 CFLAGS="-O3 -Os" CXXFLAGS="-O3 -Os" LDFLAGS="-s" \
    && strip libleveldb.so

RUN ls -sS /opt/leveldb/build